...
- Get eye to skull rotation matrix
  - Computed from measured ferret skull anatomy
  - Eyes are oriented 47.5° from skull +X (nose) in the horizontal plane and 45° above horizontal
  - Module constants `EYE_AZIMUTH_DEG` (47.5°) and `EYE_ELEVATION_DEG` (45.0°) in `calculate_ferret_gaze.py`
  - Gaze direction (eye +Z in skull frame):
    - Right eye ≈ [0.478, -0.521, 0.707]
    - Left eye  ≈ [0.478,  0.521, 0.707]
  - Up direction (eye +Y) derived via Gram-Schmidt from skull +Z
  - Left direction (eye +X = subject's left) derived via cross product
