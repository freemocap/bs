...
from python_code.ferret_gaze.eye_kinematics.ferret_eye_kinematics_serialization import \
    load_ferret_eye_kinematics_from_directory, load_ferret_eye_kinematics
...
class BlenderRecording(ABaseModel):
    ...
    def from_recording_path(cls, recording_path: str | Path) -> 'BlenderRecording':
        ...
        right_gaze_kinematics_csv = recording.right_gaze_kinematics_csv
        right_gaze_reference_gemoetry_json = recording.right_gaze_reference_geometry

        if right_gaze_kinematics_csv is None:
            raise ValueError("Right gaze kinematics not found")
        if right_gaze_reference_gemoetry_json is None:
            raise ValueError("Right gaze kinematics not found")

        right_gaze =  load_ferret_eye_kinematics(
            reference_geometry_path=right_gaze_reference_gemoetry_json,
            kinematics_csv_path=right_gaze_kinematics_csv,
        )
        left_gaze_kinematics_csv = recording.left_gaze_kinematics_csv
        left_gaze_reference_gemoetry_json = recording.left_gaze_reference_geometry

        if left_gaze_kinematics_csv is None:
            raise ValueError("Right gaze kinematics not found")
        if left_gaze_reference_gemoetry_json is None:
            raise ValueError("Right gaze kinematics not found")

        left_gaze = load_ferret_eye_kinematics(
            reference_geometry_path=left_gaze_reference_gemoetry_json,
            kinematics_csv_path=left_gaze_kinematics_csv,
        )
        ...
        return cls(
            ...
            data=BlenderData(
                ...
                right_gaze_kinematics=right_gaze,
                left_gaze_kinematics=left_gaze,
            ),
        )
