"""Recording folder model for calibration recordings (charuco board captures).

Calibration recordings are a distinct recording type from ferret sessions, not a
pipeline stage of them, so this model is kept separate from
`python_code.utilities.folder_utilities.recording_folder.RecordingFolder` rather
than folded into its `PipelineStep` machinery.
"""
