...
class GazeTracesContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "GazeTracesContext") -> bool:
    return recording_folder.gaze_kinematics is not None
...
def plot_gaze_traces(
    ...
    if not required_data_available(recording_folder, GazeTracesContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"Skipping gaze traces for {eye_name}: required data not found")
        return
