...
class SkullAndSpineTracesContext(BaseModel):
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "SkullAndSpineTracesContext") -> bool:
    return recording_folder.skull_reference_geometry is not None and recording_folder.skull_kinematics_csv is not None
...
def plot_ferret_skull_and_spine_traces(
    ...
    if not required_data_available(recording_folder, SkullAndSpineTracesContext(entity_path=entity_path)):
        print("Skipping skull and spine traces: required data not found")
        return
