...
class HeadRotationContext(BaseModel):
    entity_path: str = "/head_rotation"


def required_data_available(recording_folder: RecordingFolder, context: "HeadRotationContext") -> bool:
    if recording_folder.mocap_solver_output is None:
        return False
    rotation_csv = recording_folder.mocap_solver_output / "rotation_translation_data.csv"
...
def plot_head_rotation_for_recording(recording_folder: RecordingFolder, entity_path: str = "/head_rotation"):
    context = HeadRotationContext(entity_path=entity_path)
    if not required_data_available(recording_folder, context):
        print("Skipping head rotation: required data not found")
        return
    ...
    plot_head_rotation(video_data=topdown_mocap_video, angle_df=rotation_df, entity_path=entity_path)
...
if __name__ == "__main__":
    ...
    entity_path = "/head_rotation"

    view = get_head_rotation_view(entity_path=entity_path)
    rr.send_blueprint(rrb.Horizontal(view))

    plot_head_rotation_for_recording(recording_folder=recording_folder, entity_path=entity_path)
