...
from python_code.viz.rerun_viewer.components.gaze_traces import (
...
class NaiveGazeTracesContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "NaiveGazeTracesContext") -> bool:
    kinematics_dir = (
        recording_folder.left_eye_kinematics
        if context.eye_name == "left"
        else recording_folder.right_eye_kinematics
    )
    return kinematics_dir is not None and recording_folder.skull_reference_geometry is not None and recording_folder.skull_kinematics_csv is not None
...
def plot_naive_gaze_traces(
    ...
    if not required_data_available(recording_folder, NaiveGazeTracesContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"Skipping naive gaze traces for {eye_name}: required data not found")
        return
