...
class EyeKinematicsTracesContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "EyeKinematicsTracesContext") -> bool:
    kinematics_dir = (
        recording_folder.left_eye_kinematics
        if context.eye_name == "left"
        else recording_folder.right_eye_kinematics
    )
    return kinematics_dir is not None
...
def plot_eye_traces(
    ...
    if not required_data_available(recording_folder, EyeKinematicsTracesContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"Skipping eye kinematics traces for {eye_name}: required data not found")
        return
