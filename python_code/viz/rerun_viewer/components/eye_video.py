...
class EyeVideoContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "EyeVideoContext") -> bool:
    display_video = (
        recording_folder.left_eye_display_video
        if context.eye_name == "left"
        else recording_folder.right_eye_display_video
    )
    kinematics_dir = (
    ...
    return display_video is not None and kinematics_dir is not None
...
def plot_eye_video(
    ...
    if not required_data_available(recording_folder, EyeVideoContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"Skipping eye video for {eye_name}: required data not found")
        return
