...
class Eye3DContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "Eye3DContext") -> bool:
    kinematics_dir = (
...
def plot_3d_eye(
    ...
    if not required_data_available(recording_folder, Eye3DContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"Skipping 3D eye for {eye_name}: required data not found")
        return
