...
from python_code.viz.rerun_viewer.utils.load_tidy_dataset import load_solver_outputs
...
class Data3DContext(BaseModel):
    entity_path: str = "/tracked_object"


def required_data_available(recording_folder: RecordingFolder, context: "Data3DContext") -> bool:
    return recording_folder.mocap_solver_output is not None or recording_folder.mocap_3d_data is not None
...
def plot_data_3d(recording_folder: RecordingFolder, entity_path: str = "/tracked_object"):
    from python_code.utilities.folder_utilities.recording_folder import BaslerCamera

    context = Data3DContext(entity_path=entity_path)
    if not required_data_available(recording_folder, context):
        print("Skipping 3D data: required data not found")
        return
    ...
    if recording_folder.mocap_solver_output is not None:
        solver_output_path = recording_folder.mocap_solver_output / "skull_and_spine_trajectories.csv"
        body_data_3d = load_solver_outputs(csv_path=solver_output_path, landmarks=ferret_head_spine_landmarks)
    else:
        body_data_3d = np.load(recording_folder.mocap_3d_data / "head_body_rigid_3d_xyz.npy")
