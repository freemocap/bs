...
class TrackedPupilPoints3DContext(BaseModel):
    eye_name: Literal["left", "right"]
    entity_path: str = "/"


def required_data_available(recording_folder: RecordingFolder, context: "TrackedPupilPoints3DContext") -> bool:
    if recording_folder.gaze_kinematics is None:
        return False
    parquet_path = recording_folder.gaze_kinematics / f"{context.eye_name}_gaze_kinematics.parquet"
...
def plot_tracked_pupil_points_3d(
    ...
    if not required_data_available(recording_folder, TrackedPupilPoints3DContext(eye_name=eye_name, entity_path=entity_path)):
        print(f"  Tracked pupil points data not found for {eye_name}, skipping.")
        return
...
if __name__ == "__main__":
    ...
    from python_code.viz.rerun_viewer.components.skull_and_spine_3d import (
