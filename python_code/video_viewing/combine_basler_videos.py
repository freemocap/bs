...
def create_video_info(folder_path: Path) -> List[VideoInfo]:
    ...
    all_videos = list(
        ...
        if ("combined" not in video_path.stem and "rotated" not in video_path.stem)
    )
    print(all_videos)
    ...
    for video_path in all_videos:
        ...
        timestamps = load_synchronized_timestamps(
            folder_path, cam_name=video_path.stem.split("_")[0]
        )
...
def load_synchronized_timestamps(folder_path: Path, cam_name: str) -> np.ndarray:
    print(f"looking for cam {cam_name} timestamps")
    timestamp_path = next(folder_path.glob(f"{cam_name}*.npy"))
...
def combine_videos(
    ...
    # while frame_number < 3000:
        ...
        # combine basler into single frame
