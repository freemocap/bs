...
from python_code.eye_analysis.data_models.trajectory_dataset import Trajectory2D, ProcessedTrajectory, \
    TrajectoryDataset, DEFAULT_BUTTERWORTH_CUTOFF, DEFAULT_BUTTERWORTH_ORDER, DEFAULT_MIN_CONFIDENCE
from python_code.eye_analysis.data_processing.align_data.alignment_diagnostics import plot_correction_comparison, \
...
def apply_spatial_transform_to_trajectory(
    ...
    for frame_number in range(n_frames):
        ...
        translated_point: np.ndarray = trajectory.data[frame_number] - stabilize_on[frame_number]
...
def compute_spatial_correction_parameters(
    ...
    if n_frames != align_to.shape[0]:
        raise ValueError("stabilize_on and align_to must have the same number of frames")
    ...
    rotation_angles: np.ndarray = -np.arctan2(translated_align_to[:, 1], translated_align_to[:, 0])
    ...
    transformed_centering_offset: np.ndarray = np.median(transformed_center_on, axis=0) # (2,)
...
def apply_spatial_correction_to_dataset(
    ...
    for name, traj in dataset.trajectories.items():
        ...
        pupil_center_points = np.asarray([dataset.trajectories[pname].cleaned.data for pname in center_on])
        pupil_center_median_trajectory = np.nanmedian(pupil_center_points, axis=0)

        tear_duct_pos, rotation_angles, transformed_pupil_global_median = compute_spatial_correction_parameters(
...
def eye_alignment_main(recording_name: str, csv_path: Path, timestamps_path: Path) -> TrajectoryDataset:
...
if __name__ == "__main__":
    ...
    _recording_name: str = "2025-07-11_ferret_757_EyeCameras_P43_E15__1_0m37s-1m37s_eye1"
    eye_alignment_main(csv_path=_csv_path, timestamps_path=_timestamps_npy_path, recording_name=_recording_name)
