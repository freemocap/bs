...
class EyeVideoDataViewer(ABaseModel):
    ...
    def get_frame_points(self, *, frame_index: int) -> dict[str, np.ndarray]:
        ...
        """
        include_raw: bool = (
                self.view_config.show_raw_dots or
                self.view_config.show_raw_lines or
                self.view_config.show_raw_ellipse
        )
        include_cleaned: bool = (
                self.view_config.show_cleaned_dots or
                self.view_config.show_cleaned_lines or
    ...
    def get_frame_info(self, *, frame_index: int) -> dict[str, object]:
        ...
        view_mode: str = "both" if (
                self.view_config.show_raw_dots and self.view_config.show_cleaned_dots
        ) else "cleaned" if self.view_config.show_cleaned_dots else "raw"
    ...
    def run(self, *, start_frame: int = 0, save_dir: Path | None = None) -> None:
        ...
        while True:
            ...
            if key == ord('q') or key == 27:
                ...
                self.playback_controls.skip_frames = max(0, self.playback_controls.skip_frames - 1)
                print(f"Frame skip: {self.playback_controls.skip_frames}")
            elif paused:
                if key == 83:
                    current_frame = min(
