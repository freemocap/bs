...
class RecordingFolder(BaseModel):
    ...
    def create_from_clip(cls, 
        ...
        for path in [eye_data_folder, eye_annotated_videos_folder, eye_synchronized_videos_folder, eye_timestamps_folder, eye_output_data_folder]:
            if not path.exists():
        ...
        right_eye_video_path = list(eye_synchronized_videos_folder.glob("eye1*.mp4"))[0]
        ...
        left_eye_video_path = list(eye_synchronized_videos_folder.glob("eye0*.mp4"))[0]
        ...
        for path in [mocap_data_folder, mocap_annotated_videos_folder, mocap_synchronized_videos_folder, mocap_timestamps_folder, mocap_output_data_folder]:
            if not path.exists():
                raise ValueError(f"Path does not exist: {path}")

        mocap_csv_path = list(mocap_output_data_folder.glob("skellyclicker_machine_labels*.csv"))[0]
